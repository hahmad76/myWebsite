...
async function closeTestServer({ tempDir, server }) {
...
test("health endpoint reports ok", async () => {
  ...
  try {
    const result = await request(ctx.server, "GET", "/api/health");
...
test("service request is validated and persisted", async () => {
  const ctx = await makeTestServer();
  try {
    const result = await request(ctx.server, "POST", "/api/service-requests", {
      service_select: "School Documentation", name: "Test User", phone: "+923001234567", requirement: "Test request"
    });
    assert.equal(result.status, 201);
    const saved = await ctx.storage.list("serviceRequests");
    assert.equal(saved.length, 1);
    assert.equal(saved[0].type, "Service Request");
    assert.equal(saved[0].data.name, "Test User");
    const audit = await ctx.storage.list("auditLog");
    assert.equal(audit.length, 1);
  } finally { await closeTestServer(ctx); }
});

test("invalid service request is rejected", async () => {
  const ctx = await makeTestServer();
  try {
    const result = await request(ctx.server, "POST", "/api/service-requests", { name: "Incomplete" });

...
if(($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'POST')sr_json(405,['success'=>false,'error'=>'Method not allowed']);
...
try{
    ...
    try{
        ...
        if(!$sent)error_log('SSHP service-request email could not be handed to the hosting mail system for request '.$id);

...
if (isset($body['source_page'])) {
    $fields['Source Page'] = $body['source_page'];
}

if (isset($body['submitted_at'])) {
    $fields['Submitted At'] = $body['submitted_at'];
}

$subject = 'Universal Customer Submission';

if (isset($body['subject']) && trim((string)$body['subject']) !== '') {
    $subject = trim((string)$body['subject']);
}

try {
    $sent = sshp_mail_owner(
        $subject,
        'NEW WEBSITE CUSTOMER SUBMISSION',
        $fields
    );
    ...
    error_log(
        'SSHP universal owner email error: ' . $e->getMessage()
    );
